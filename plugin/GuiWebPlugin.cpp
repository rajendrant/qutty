...
GuiWebPlugin::GuiWebPlugin(GuiMainWindow *p)
    : QToolBar(p), mainWnd(p), web(this), currWebTerm(mainWnd->getCurrentTerminal()) {
  addWidget(&web);
...
QSize GuiWebPlugin::sizeHint() const { return QSize(mainWnd->width() / 3, mainWnd->height() / 3); }

void GuiWebPlugin::resizeMe(int w, int h) { this->resize(w, h); }
...
void GuiWebPlugin::on_loadFinished(bool ok) {
  web.setFocus();
  this->move((mainWnd->width() - this->width()) / 2, (mainWnd->height() - this->height()) / 2);
}

QVariantList GuiWebPlugin::getPersistentList(QString key) {
  if (qutty_web_plugin_map.hash_map.contains(key)) return qutty_web_plugin_map.hash_map[key];
  return QVariantList();
}
